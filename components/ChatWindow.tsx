...
import { Slider, Checkbox, Flex } from '@radix-ui/themes';
...
export function ChatWindow(props: {
  ...
  const START = "At the start of the conversation"
  const MIDDLE = "Weaved in the middle"
  const END = "At the conclusion"
  const [adPositions, setAdPositions] = useState<string[]>([START, MIDDLE, END]);
  const handleAdPositionCheckboxChange = (value: string) => {
    setAdPositions(prevSelectedValues => {
      if (prevSelectedValues.includes(value)) {
        return prevSelectedValues.filter(val => val !== value);
      } else {
        const newArr = [...prevSelectedValues, value];
        return newArr;
      }
    });
  };
  const isCheckboxChecked = (value: string) => adPositions.includes(value);
  ...
  const adPositionsToggle = (
    <div>
      <Flex gap="2">
        Ad positions:
        <label>
          Start of the conversation<Checkbox color="indigo" variant='surface' checked={isCheckboxChecked(START)} onCheckedChange={() => {handleAdPositionCheckboxChange(START)}}/>
        </label>
        <br/>
        <label>
          Weaved in the middle <Checkbox color="cyan" variant='surface' checked={isCheckboxChecked(MIDDLE)} onCheckedChange={() => handleAdPositionCheckboxChange(MIDDLE)} />
        </label>
        <label>
          At the conclusion <Checkbox color="orange" variant='surface' checked={isCheckboxChecked(END)} onCheckedChange={() => handleAdPositionCheckboxChange(END)} />
        </label>
      </Flex>
    </div>
  )
  ...
  const adToneSliders = (
      <div>
        <Flex direction="column" gap="4" style={{ maxWidth: 450 }}>
          <label>Humor 😂<Slider variant="surface" defaultValue={[75]} /> </label>
          <label>Formality 🎩</label><Slider variant="surface" defaultValue={[50]} />
          <label>Seduction ❤️‍🔥<Slider variant="surface" defaultValue={[25]} /></label>
        </Flex>
      </div>
  )
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          <input
            ...
            value={input}
            placeholder={placeholder ?? "Ask your questions and get answers thanks to our sponsors"}
